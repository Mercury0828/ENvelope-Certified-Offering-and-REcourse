...
holds from commitment to metering.
...
the rolling controller, so Theorem~\ref{thm:main} holds through settlement.}
\label{fig:ctrl_arch}
\vspace{-3mm}
\end{figure}
...
is a half-hour resource whose size is weather-coupled.
...
hours directly; it does not assume that coverage transfers.
...
The ball $\bar e_0$ is set by what the uncommitted hour of
...
start outside the ball; the validation of
Section~\ref{sec:experimental_evaluation} counts these starts. A certificate built on
...
The guarantee never depends on the online layer: the fallback policy itself
carries the certificate, the opposite of robustifying the online controller
as in~\cite{shen2026cdro}.
...
therefore limited by information (the context), not by computation.
