...
the plant there before each obligation. Fig.~\ref{fig:slices} shows the
certified object directly: exact slices of $\mathcal{D}(c)$ in the state
plane, i.e., the startable sets $\mathcal{R}(q)=\{x:F(x,c)\ge q\}$ for three
...
the calibrated model sustains $53$--$59\%$ of baseline cooling power for
$d=30$\,min from the nominal operating state, and $90\%$ from the pre-cooled
ready state on a dry day; the envelope collapses toward zero as the
dew-point forecast approaches the supply floor. The thermal mass is a
genuine half-hour resource whose size is weather-coupled.
