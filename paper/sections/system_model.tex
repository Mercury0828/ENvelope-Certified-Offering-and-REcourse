...
The disturbance is
\begin{equation}
\xi_t = [w_t,\; \delta^{\mathrm{dew}}_t]^\top ,
\end{equation}
where $w_t$ is the residual of the IT heat load around its day-ahead forecast
$\bar Q$, driven by bursty, non-Gaussian AI workload dynamics, and
$\delta^{\mathrm{dew}}_t$ is the dew-point forecast error, which shifts the
anti-condensation boundary. The day-ahead context vector
\begin{equation}
...
tranche (time constant near an hour), and is what makes a half-hour product
physically possible:
\begin{equation}
...
All nonlinearity is confined to the state-dependent input set
\begin{equation}
\mathcal{U}(x;c)=\Bigl\{u:
\begin{aligned}
&0 \le \uext \le \mdot c_p\bigl(\Tw-\vartheta(c)\bigr),\\
...
\label{eq:Ux}
\end{equation}
which is \emph{affine} in $x$ for a fixed context. The first bound carries
...
chiller equipment limit. Sub-minute chip-level bursts are summarized by
...
$\eta$ (ref.\ COP) & supply-sweep fit & $\Delta t$ / $\dH$ & $5$\,min / $1$\,h \\
\bottomrule
\end{tabular}
\vspace{-3mm}
\end{table}
...
parameter fixed per market (we evaluate $d=30$); activation is exogenous with
probability $p_{\mathrm{act}}$ and activated fraction $r_h\in[0,1]$,
$r_h\cdot 60\le d$. The product is stylized: it maps loosely onto
short-duration reserve and emergency products without encoding any single
tariff, and it is the only institutional element in the model; all other
market realism enters through data. Hour $h$ settles on metered energy
...
hotspot elevation. Whole-hour metering in \eqref{eq:settlement} is
load-bearing: recovery \emph{within} the activated hour cancels delivered
...
phenomenon in demand-response programs~\cite{chao2011demand}; it is orthogonal
to deliverability and excluded by construction, as in the buildings-reserves
literature~\cite{vrettos2016robust}.
\end{remark}
...
unsafe or wasteful across contexts. This is the dilemma that motivates
context-dependent robustness in the companion control
problem~\cite{shen2026cdro}, transposed here to the market layer.
