...
side} of the liquid-cooled AI facility. Direct-to-chip liquid cooling
...
distributed resources, the analogous problem is resolved by the do-not-exceed
limit~\cite{zhao2015variable} and the dynamic operating
envelope~\cite{aemo2022doe}; no counterpart exists for a thermal resource
whose limits move with the weather.
...
uncertainty for the average case lets safety erode exactly when the product is
exercised. The commitment therefore needs \emph{two} guarantees with two
different confidence levels, and a real-time layer simple enough to run on
facility control hardware that nevertheless inherits them.
...
& \textbf{Exact certified envelope}
...
buildings: robust capacity allocation ahead of time, an online MPC layer, and
experimental market
...
region rather than a point forecast. ENCORE is the thermal-resource member of
...
Our goal is to let a liquid-cooled AI data center sell its cooling-side
thermal inertia in a day-ahead market with guarantees that survive to
real-time delivery. A deterministic offer is unsafe and a static worst-case
offer is worthless, so the offer must come from a context-dependent envelope
robustified at two confidence levels with different stakes. The main
...
settlement. A certified fallback policy underwrites the real-time layer. We prove that any
...
\item We validate the certificate end-to-end on measured data only: a
2.0-million-worker GPU-cluster trace with a causal job-aware day-ahead
forecast, archived numerical-weather-prediction dew-point forecasts, and ten
2024 ERCOT weeks. Across $1{,}400$ controller-days the certified participant
incurs zero in-domain violation episodes (against $229$ violation-days for
the uncertified envelope), delivery failures stay inside the priced budget,
and day-ahead workload information alone raises a real hall's certifiable
offer by $83\%$.
\end{itemize}
